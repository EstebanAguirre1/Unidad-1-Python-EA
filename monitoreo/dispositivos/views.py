...
def listado_dispositivos(request):
    # Traer todas las categorías como objetos (para mostrar nombre en vez de ID)
    categorias = Category.objects.all()

    # Filtrar por categoría si se recibe por GET
    categoria_seleccionada = request.GET.get('categoria')
    if categoria_seleccionada:
        dispositivos = Device.objects.filter(category_id=categoria_seleccionada)
    else:
...
def detalle_dispositivo(request, device_id):
    # Obtener el dispositivo, o mostrar 404 si no existe
    device = get_object_or_404(Device, id=device_id)

    # Obtener mediciones y alertas asociadas al dispositivo
